...
class settings(commands.Cog):
    ...
    async def on_guild_join(self, guild):
        ...
        params = (id, 1)
        cursor.execute(
            "INSERT OR IGNORE INTO guilds (guild_id, x_said) VALUES (?, ?)", params
        )
        connection.commit()
        connection.close()

    @commands.hybrid_command(
        name="set", description="sets the text channel that voicely will read from"
    )
    ...
    async def set(self, ctx: commands.Context, channel: discord.TextChannel) -> None:
        ...
        cursor.execute(
            "UPDATE guilds SET text_channel_id= ? WHERE guild_id= ? ", (tc, id)
        )
    ...
    @commands.hybrid_command(
        name="xsaid", description="enables/disables x said prefix when speaking"
    )
